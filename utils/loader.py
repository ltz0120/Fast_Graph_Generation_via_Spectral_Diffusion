...
from losses import get_sde_loss_fn, get_sde_loss_fn2
from solver import get_pc_sampler, get_pc_sampler2
...
def load_device():
    if torch.cuda.is_available():
        ...
        indexes = [i for i, x in enumerate(used_list) if x == min(used_list)]
        print("use device:",indexes[0])
        device = [indexes[0]]
...
def load_model(params):
    ...
    elif model_type == 'ScoreNetworkA_eigen':
        model = ScoreNetworkA_eigen(**params_)
    else:
...
def load_data2(config, get_graph_list=False):

    from utils.data_loader import dataloader2
    return dataloader2(config, get_graph_list)
...
def load_batch2(batch, device):
    device_id = f'cuda:{device[0]}' if isinstance(device, list) else device
    x_b = batch[0].to(device_id)
    adj_b = batch[1].to(device_id)
    u_b = batch[2].to(device_id)
    la_b = batch[3].to(device_id)
    return x_b, adj_b, u_b, la_b
...
def load_sampling_fn2(config_train, config_module, config_sample, device):
    sde_x = load_sde(config_train.sde.x)
    sde_adj = load_sde(config_train.sde.adj)
...
def load_model_params(config):
    ...
    if 'GMH' in config_m.x:
        params_x = {'model_type': config_m.type, 'max_feat_num': max_feat_num, 'depth': config_m.depth, 
                    ...
                    'adim': config_m.adim, 'num_heads': config_m.num_heads, 'conv':config_m.conv, 'max_node_num':config.data.max_node_num}
    else:
        if 'eigen' in config_m.x:
            params_x = {'model_type': config_m.x, 'max_feat_num': max_feat_num, 'depth': config_m.depth,
                        'nhid': config_m.nhid,'max_node_num':config.data.max_node_num}
        else:
...
def load_model_params_u(config):
    config_m = config.model
    max_feat_num = config.data.max_feat_num
    ...
    params_adj = {'model_type':config_m.adj, 'max_feat_num':max_feat_num, 'max_node_num':config.data.max_node_num,
                    ...
                    'c_init':config_m.c_init, 'c_hid':config_m.c_hid, 'c_final':config_m.c_final,
                    'adim':config_m.adim, 'num_heads':config_m.num_heads, 'conv':config_m.conv}
    params_u = {'model_type': config_m.u, 'max_feat_num': max_feat_num, 'max_node_num': config.data.max_node_num,
                  'nhid': config_m.nhid, 'num_layers': config_m.num_layers, 'num_linears': config_m.num_linears,
                  'c_init': config_m.c_init, 'c_hid': config_m.c_hid, 'c_final': config_m.c_final,
                  'adim': config_m.adim, 'num_heads': config_m.num_heads, 'conv': config_m.conv}
    return params_x, params_adj, params_u
...
def load_ckpt_u(config, device, ts=None, return_ckpt=False):
    device_id = f'cuda:{device[0]}' if isinstance(device, list) else device
    ckpt_dict = {}
    if ts is not None:
        config.ckpt = ts
    ...
    ckpt = torch.load(path, map_location=device_id)
    print(f'{path} loaded')
    ckpt_dict= {'config': ckpt['model_config'], 'params_x': ckpt['params_x'], 'x_state_dict': ckpt['x_state_dict'],'params_u': ckpt['params_u'], 'u_state_dict': ckpt['u_state_dict'],
                'params_adj': ckpt['params_adj'], 'adj_state_dict': ckpt['adj_state_dict']}
    if config.sample.use_ema:
        ckpt_dict['ema_x'] = ckpt['ema_x']
        ckpt_dict['ema_adj'] = ckpt['ema_adj']
...
def load_ckpt2(config, device, ts=None, return_ckpt=False):
    device_id = f'cuda:{device[0]}' if isinstance(device, list) else device
    ckpt_dict = {}
    if ts is not None:
        config.ckpt = ts
